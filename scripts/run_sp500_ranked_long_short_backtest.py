#!/usr/bin/env python3
"""Run S&P 500 ranked long/short stock trend portfolios.

Portfolio construction requested by the user:

- Use stock-only forecasts; no futures carry or roll signal.
- Rank current S&P 500 stocks by combined EWMAC + breakout forecast.
- Hold the highest ranked stocks long and lowest ranked stocks short.
- Test 20-stock and 40-stock versions.
- Allocate 70% long / 30% short by gross capital, so net exposure is +40%.

The data source is the same yfinance/Wikipedia cache used by
run_sp500_stock_trend_backtest.py. Using today's S&P 500 members back to 2000
has survivorship and membership look-ahead bias.
"""
...
OUT = ROOT / "backtests" / "sp500_ranked_long_short"
...
def build_rank_weights(forecast: pd.DataFrame, total_names: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    long_count = int(round(total_names * LONG_FRACTION))
    short_count = total_names - long_count
    if long_count <= 0 or short_count <= 0:
        raise ValueError("total_names must produce at least one long and one short")
    ...
    for date in rebalance_dates:
        ...
        long_names = scores.nlargest(long_count).index
        short_names = scores.nsmallest(short_count).index
        target_on_rebalance.loc[date, long_names] = LONG_FRACTION / long_count
        target_on_rebalance.loc[date, short_names] = -SHORT_FRACTION / short_count
            ...
            selection_rows.append(
                {
                    ...
                    "weight": LONG_FRACTION / long_count,
                }
            )
        for rank, ticker in enumerate(short_names, start=1):
            selection_rows.append(
                {
                    "date": date,
                    "portfolio_size": total_names,
                    ...
                    "ticker": ticker,
                    "forecast": float(scores[ticker]),
                    "weight": -SHORT_FRACTION / short_count,
                }
            )
...
def run_rank_portfolio(price: pd.DataFrame, forecast: pd.DataFrame, total_names: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    weights, selections = build_rank_weights(forecast, total_names)
...
def plot_results(streams: dict[str, pd.Series], annual: pd.DataFrame, corr20: pd.Series, corr40: pd.Series) -> None:
    ...
    colors = {
        "Rank 20 70/30": "#1f77b4",
        "Rank 40 70/30": "#ff7f0e",
        "SPY": "#4c4c4c",
    }
    ...
    ax2.plot(corr20.index, corr20, color=colors["Rank 20 70/30"], label="Rank 20 to SPY", linewidth=1.2)
    ax2.plot(corr40.index, corr40, color=colors["Rank 40 70/30"], label="Rank 40 to SPY", linewidth=1.2)
    ax2.axhline(0.0, color="#777777", linewidth=0.8)
    ax2.legend(loc="upper left", ncol=2)

    annual = annual[["Rank 20 70/30", "Rank 40 70/30", "SPY"]].dropna(how="all")
    ...
    ax0.set_title("S&P 500 Ranked Stock Trend 70/30 vs SPY")
    ...
    fig.tight_layout()
    fig.savefig(OUT / "ranked_70_30_20_40_vs_spy.png", dpi=180)
    plt.close(fig)
...
def write_summary(stats: pd.DataFrame, crisis: pd.DataFrame, usable_count: int, latest_price_date: pd.Timestamp) -> None:
    lines = [
        "# S&P 500 Ranked Long/Short Trend Backtest",
        "",
        ...
        "- Portfolio: weekly rebalance; 70% long / 30% short; 20-stock version holds 14 long and 6 short; 40-stock version holds 28 long and 12 short.",
...
def main() -> None:
    ...
    daily20, weights20, selections20 = run_rank_portfolio(price, forecast, 20)
    daily40, weights40, selections40 = run_rank_portfolio(price, forecast, 40)

    ret20 = trim_to_first_position(daily20["net_return"].rename("Rank 20 70/30"), weights20)
    ret40 = trim_to_first_position(daily40["net_return"].rename("Rank 40 70/30"), weights40)
    ...
    streams = {
        "Rank 20 70/30": ret20.loc[common_start:common_end],
        "Rank 40 70/30": ret40.loc[common_start:common_end],
    ...
    corr20 = rolling_corr(streams["Rank 20 70/30"], streams["SPY"])
    corr40 = rolling_corr(streams["Rank 40 70/30"], streams["SPY"])

    daily20.to_csv(OUT / "portfolio_daily_rank20.csv")
    daily40.to_csv(OUT / "portfolio_daily_rank40.csv")
    weights20.iloc[::5].to_csv(OUT / "weekly_weights_rank20.csv")
    weights40.iloc[::5].to_csv(OUT / "weekly_weights_rank40.csv")
    ...
    pd.DataFrame({"Rank 20 to SPY": corr20, "Rank 40 to SPY": corr40}).to_csv(OUT / "rolling_corr_to_spy.csv")
