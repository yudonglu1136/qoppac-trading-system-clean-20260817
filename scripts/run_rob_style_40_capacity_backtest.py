#!/usr/bin/env python3
"""Run a 40-instrument Rob-style system sized for a USD 500k account."""
...
OUT = bt.ROOT / "backtests" / "rob_style_40_capacity"

# Chosen using ex-ante capacity diagnostics, not realised P&L:
# Rob config weight, local data coverage, rules available, cost/risk, and
# median contracts for a forecast=10 position under a USD 500k account.
CAPACITY_UNIVERSE = {
    "Equity": [
        "SP500_micro",
        "NASDAQ_micro",
...
def selected_instruments() -> list[str]:
    return [instrument for instruments in CAPACITY_UNIVERSE.values() for instrument in instruments]
        ...
        rows.append(
            {
                ...
                "selection_bucket": next(bucket for bucket, names in CAPACITY_UNIVERSE.items() if instrument in names),
...
def plot_results(
    ...
    axes[0].set_title("Rob-Style 40-Instrument Capacity Portfolio Equity")
...
def write_summary(
    ...
    lines = [
        "# Rob-Style 40-Instrument Capacity Backtest",
        "",
        "## Design",
        "",
        "- Keeps Rob's 40 configured rule variations, forecast scalars, forecast weights, and FDM values.",
        "- Uses a 40-instrument universe selected for a USD 500,000 account using ex-ante capacity/cost diagnostics.",
        "- Renormalises Rob's instrument weights across the selected 40 markets.",
        "- Runs both continuous target positions and buffered integer positions.",
        "- Excludes high-friction or very small-allocation markets such as dairy, most sector futures, OMX, MSCIWORLD, and VNKI.",
        "",
        "## Universe",
        "",
    ]
    for bucket, names in CAPACITY_UNIVERSE.items():
        lines.append(f"- {bucket} ({len(names)}): {', '.join(names)}")
    lines.extend(
        [
            "",
    ...
    for row in low_capacity.itertuples(index=False):
        lines.append(
            f"- {row.instrument} ({row.asset_class}): median forecast=10 position {row.median_contracts_at_forecast_10:.2f}; median cost/risk {row.median_cost_as_pct_annual_risk:.2%}"
        )
    ...
    lines.extend(
        [
            ...
            "- The universe selection uses capacity and cost metrics, not future P&L, but it is still a research design choice.",
            "- Asset-class relative rules are computed inside this 40-instrument universe.",
            "",
        ]
    )
